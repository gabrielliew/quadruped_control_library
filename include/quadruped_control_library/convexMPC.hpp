...
public:
  ...
  /**
   * @brief Updates the body inertia of the robot in the case of added payload
   * that significantly modifies robot inertia
   *
   * @param IBody The simplified body inertia of the robot with size <3,3>
   */
  void updateBodyInertia(Eigen::Matrix<double, 3, 3> IBody) { IBody_ = IBody; }
   ...
   */
  void updateBodyMass(double mass) { mass_ = mass; }
   ...
   */
  void updateFrictionCoefficient(double mu) { mu_ = mu; }

  void updateMaxForce(double maxForce) { maxForce_ = maxForce; }
  /**
  ...
  solveMPC(const MPCdata &currentState, const MPCdata &desiredState,
  ...
  getHRed_gRed(const Eigen::Matrix<double, -1, -1> &H,
