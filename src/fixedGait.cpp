...
Gait::Gait(uint numLegs, uint numSegments, double gaitDuration,
...
std::vector<double> Gait::getContactProgression(uint64_t timeFromStart) {
  ...
  for (int i = 0; i < numLegs_; i++) {
    ...
    0 is swing, 1 is contact so the progress has to be shifted to normalize it
    to 1111100000 at each leg but shift segment progress
    */
    ...
    if (normalizedTimeSegment <= contactDurationNs_) {
      tempProgress.push_back(static_cast<double>(normalizedTimeSegment) /
                             contactDurationNs_);
    } else {
...
std::vector<double> Gait::getSwingProgression(uint64_t timeFromStart) {
  // gets current time segment progress
  uint64_t currentTime = timeFromStart % gaitDurationNs_;
  std::vector<double> tempProgress;
  for (int i = 0; i < numLegs_; i++) {
    ...
    if (normalizedTimeSegment > contactDurationNs_) {
      tempProgress.push_back(
          static_cast<double>(normalizedTimeSegment - contactDurationNs_) /
          swingDurationNs_);
    } else {
      tempProgress.push_back(0);
    }
...
std::vector<bool> Gait::getGaitTable(uint64_t timeFromStart) {
